...
class ServerTCP:
    def __init__(self, server_port, host='0.0.0.0'):
        self.is_shutting_down = False  # This flag tracks if the server is shutting down
        self.server_port = server_port
        self.host = host
        self.server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.server_socket.bind((self.host, self.server_port))  # Bind to all interfaces (0.0.0.0) for online access
...
class ClientTCP:
    def __init__(self, client_name, server_port, server_host='localhost'):
        # Initialize the client, set the server address, and prepare the socket for connection
        self.server_addr = server_host  # Allow connecting to any server IP
